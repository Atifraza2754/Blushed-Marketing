...
@section('content')
                ...
                <div class="row">
                    <div class="col-lg-12 ">
                        <div class="d-flex justify-content-between align-items-center flex-md-row flex-column mb-3">
                            <h1 class="f-22 w-600 text-bl">Infos</h1>
                                ...
                                <tbody>
                                    @foreach ($infos as $info)
                                        <tr class="">
                                            ...
                                            <td>
                                                <p class="mb-0 pb-0">{{ $info->title }}</p>
                                            </td>
                                            <td>
                                                <p class="mb-0 pb-0">{{ $info->brand->title }}</p>
                                            </td>
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <p class="mb-0 pb-0">{{ $info->created_at->toDateString() }}</p>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="d-flex align-items-center justify-content-end">
                                                    <div class="edit table-action cursor-pointer">
                                                        <a href="{{ URL::to("/learning-center/info/$info->id/edit") }}">
                                                    ...
                                                    <div class="delete table-action cursor-pointer">
                                                        <a onclick="deletefunction({{ $info->id }})"
    ...
    {{-- include brand modal --}}
    @include('learning-center.infos.brands-modal')
@endsection
    ...
    <script>
        $(document).ready(function() {
            $('.learning-table').DataTable({
                paging: true,
                language: {
                    searchPlaceholder: "Search infos..."
                }
            });
        });
...
Swal.fire({
    title: 'Are you sure To Confirm This Info?',
    ...
    if (result.isConfirmed) {
        ...
        $.ajax({
            ...
            type: "get",
            url: "/learning-center/info/" + id + "/delete",
