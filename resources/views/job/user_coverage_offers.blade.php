@extends('user.layouts.master', ['module' => 'coverage'])
                        ...
                        <div class="col-md-4 col-sm-6 mb-4">
                            <div class="counter-box rounded"
                                ...
                                <div class="d-flex justify-content-around mt-3">
                                    ...
                                    @elseif($shift->status == 'reject')
                                        <button class="btn  btn-sm text-danger" disabled>
                                            {{strtoupper($shift->status) ?? 'REJECTED'}}
                                        </button>
                                    @elseif($shift->status == 'can_if_needed' || $shift->status == 'unable')
                                        <button class="btn  btn-sm text-dark" disabled>
                                            {{'Can, If Needed'}}
                                        </button>
                                            @if(!in_array( $shift->id , $check_coverage))
                                        <button class="btn  btn-sm text-dark border btn_cl " onclick="sendReq({{ $shift->coverage_request_id }})">
                                            Request Job
                                        </button>
                                        @else
                                        <button class="btn  btn-sm text-dark border btn_cl " disabled>
                                            Requested
                                        </button>
                                        @endif
                                    @else
                                        <!-- Accept Button -->
                                        <button class="btn btn-sm text-success" onclick="acceptShift('{{ $shift->id }}')">
                                            Accept
                                        </button>
                                        <!-- Decline Button -->
                                        <button class="btn btn-sm text-danger" onclick="declineShift('{{ $shift->id }}')">
                                            Reject
                                        </button>
                                    @endif
                                </div>
...
@section('customScripts')
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.status == 200) {
                                Swal.fire({
                                    title: "Thank you!",
                                    html: `Thank you for confirming your shift on <b>${response.date}</b> at <b>${response.time}</b> for <b>${response.brand}</b> at <b>${response.account}</b>. <br><br>You have <b>${response.quiz_count}</b> quizzes due.<br><a href="${response.quiz_link}" target="_blank">Take Quizzes</a>`,
                                    icon: 'success',
                                    confirmButtonText: 'OK'
                                }).then(() => {
                                    location.reload();
    ...
    Swal.fire({
        title: 'Are you sure?',
        text: "Do you want to offer to cover this shift?",
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: 'Yes, I can cover it',
        cancelButtonText: 'Cancel'
    }).then((result) => {
        if (result.isConfirmed) {
            ...
            $.ajax({
                type: "GET",
                url: "/user/shifts/coverage/accept/" + id,
